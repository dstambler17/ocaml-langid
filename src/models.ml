...
let load_fst_map (file_path : string) : (int, int list, 'a) Map.t =
  ...
  let open Yojson.Basic.Util in
  let json_item = load_json_string file_path in
  json_item |> member "tk_output" |> to_assoc
  |> List.map ~f:(fun (k, v) -> (int_of_string k, v |> to_list |> filter_int))
  |> create_map_helper

(* Loads .npy (numpy model) files *)
let load_model_file (path : string) : arr = O.load_npy path

let get_state_count_map (input_str : string) (tk_nextmove : int list) :
    (int, int, 'a) Map.t =
    ...
    |> String.fold
         ~f:(fun (state_count_map, state) letter ->
           ...
           let cur_count =
             ...
             | None -> 0
           in
           ( Map.set state_count_map ~key:cur_state ~data:(cur_count + 1),
             cur_state ))
         ~init:(state_count, 0)
  in
  state_map
...
let get_index_count_list state_map (tk_output : (int, int list, 'a) Map.t) :
    (int * int) list =
  Map.keys state_map
  |> List.fold
       ~f:(fun idx_list state ->
         let index_sub_list =
           match Map.find tk_output state with
           | Some list_item -> list_item
           | None -> []
         in
         let tuple_list =
           index_sub_list
           |> List.map ~f:(fun cur_index ->
                  let cur_val_count =
...
let instance2fv (input_str : string) (tk_nextmove : int list)
    (tk_output : (int, int list, 'a) Map.t) : arr =
  ...
  let _ =
    index_count_tuple_list
    |> List.iter ~f:(fun (idx, count_val) ->
           let cur_val = O.get feature_vec (List.to_array [ 0; idx ]) in
           let _ =
             O.set feature_vec
               (List.to_array [ 0; idx ])
               (cur_val +. float_of_int count_val)
           in
           ())
  in
  feature_vec
...
let norm_probs (inp : arr) : arr = O.softmax inp

let owl_1d_array_to_list (a : arr) : 'a list =
  O.shape a |> Fn.flip Array.get 1
  |> List.init ~f:(fun x -> x)
